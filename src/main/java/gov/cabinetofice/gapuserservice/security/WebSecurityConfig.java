...
public class WebSecurityConfig {
        ...
        /**
         * Using WebSecurityCustomizer#ignoring triggers a warning at app start-up for
         * each path ignored.
         * Unfortunately, its recommendation is not suitable for us, since we not only
         * need authentication to be ignored
         * but also the JWT filter - permitAll via HttpSecurity#authorizeHttpRequests
         * will still trigger the JwtTokenFilter
         * One alternative is to create two SecurityFilterChain beans, one for public
         * paths and another for secured.
         * But the secured path then cannot use anyRequest(): you must specify every
         * path you want authenticated.
         */
        @Bean
        ...
        public SecurityFilterChain filterChainPublic(final HttpSecurity http) throws Exception {
                // disable session creation by Spring Security, since auth will happen on every
                // request
                http
                                .sessionManagement()
                                .sessionCreationPolicy(SessionCreationPolicy.STATELESS);

                // any requests to any path must be authenticated (unless specified in
